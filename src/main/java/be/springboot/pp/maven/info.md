...
- **Using JAR Files:** Download a JAR file and add it to the project (e.g., to add extra JAR in IntelliJ IDEA: File > Project Structure > Libraries. Those files will be added in "External libraries" folder and will be placed on the application classpath).
  - **Drawbacks:** You are doing dependency management. Manually downloading and including subsequent JAR dependencies.
...
## Introduction
Maven is a build automation and dependency management tool. It simplifies project management by handling dependencies and their classpath. Managing project structure and versions. Automating tasks like compilation, packaging, and deployment. Also manages repositories and plugins, streamlining a project's lifecycle.
...
- **Repositories:** Maven retrieves dependencies from repositories -
  ...
  - **Central repository:** The default remote repository hosted on the internet at domain https://mvnrepository.com/. We can copy dependency details from this site and put it in our pom.xml file and maven will download and include that dependency on classpath for us.
...
## core maven commands
- `mvn clean`: removes all files generated by previous builds in `target/` directory.
- `mvn compile`: compiles the source code
- `mvn test`: runs the unit tests.
- `mvn package`: packages the project into `JAR/WAR`
- `mvn install`: installs package into your local (`.m2`) repository
- `mvn deploy`: deploys the project to a remote repository.
- `mvn spring-boot:run`: runs the Spring Boot application (if configured)
